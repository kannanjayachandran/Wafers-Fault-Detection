...
class TestBestModel(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.file_object = 'test.log'

    def test_find_best_pipeline(self):
        # Test that the best pipeline is not None
        model = BestModel(self.x_train, self.y_train, self.x_test, self.y_test, self.file_object)
    ...
    def test_evaluate_best_model(self):
        ...
        model.find_best_pipeline()
        model.train_best_model()
        accuracy = model.evaluate_best_model()
    ...
    def test_save_load_model(self):
        ...
        loaded_model = BestModel(self.x_train, self.y_train, self.x_test, self.y_test, self.file_object)
        loaded_model.load_model('test_model.joblib')
        loaded_accuracy = loaded_model.evaluate_best_model()
